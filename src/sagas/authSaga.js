...
function* signIn({ payload }) {
  try {
    ...
    if (response.error === 'New password required') {
      ...
      localStorage.setItem('user.username', response.idToken.payload.email);
      ...
      yield put({
        ...
        payload: {
          url: `/merchants/?UserId=${response.idToken.payload.email}`,
          success: (res) => {
            if (get(res, 'data.0.status.code') === '1008' || get(res, 'data.0.status.code') === '1006') {
