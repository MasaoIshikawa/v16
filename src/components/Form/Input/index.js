...
class Input extends Component {
  ...
  render() {
    const { className, labelClassName, notification, isMasked, isBadgeVisible, errorMessage, label, placeHolder, value = '', hasError, hasValue, isDisabled, isRequired, ...restProps } = this.props;
    const { isFocused, isNotificationShowing } = this.state;

    return (
      <label className={cn({ 'has-value': isFocused || value.length || isDisabled || hasError, required: this.state.isRequired || hasError }, labelClassName)}>
        {
          label &&
            <span>
              {label}
        ...
        {
          notification &&
          <div className={cn(style.tooltip, isNotificationShowing && style.active)}>
            <i
        ...
        <div className={style.error}>{errorMessage !== 'The input field is required' && errorMessage}</div>
      </label>
    );
  }
}
